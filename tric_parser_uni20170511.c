...
/* BUBBLING UNIPROCESSOR VERSION, NO SNOWFLAKE */
      ...
      for (n = 0; n < leninput - 1; n++) {
        ...
        for (i = 0; i < leninput; i++) {
          ...
          for (j = 0; j < atomcount; j++) {
            ...
            /* The multicore version has instead (without the matching brace) */
            /* a "head-heavy" variant instead of above "tail-heavy": */
            /* (atoms[j][atomvalue] < candidatevalue)) {} */
