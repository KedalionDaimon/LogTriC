...
long atomeffects[maxtri];
long atomeffectssum[maxtri];
long atomeffectsdivisor;
long conjugateeffects[maxtri];
long cohabitanteffects[maxtri];
...
/* ____START____OF____LOGICAL____TRIANGULATION____ */
  ...
  */
  ...
  for (h = 0; h < sledge; h++) {
    ...
    /*
    printf("sledge: %d, reconsiderations: %d\n", h, l);
    */
...
#pragma omp barrier
          ...
          for (i = 1; i < atomcount; i++) {
            ...
            if ((atoms[0][leftatom] == atoms[i][leftatom])    &&
              ...
              conjugate[i][atomnumber]      = atoms[i][atomnumber];
              conjugate[i][leftatom]        = atoms[i][leftatom];
              conjugate[i][rightatom]       = atoms[i][rightatom];
              conjugate[i][atomvalue]       = atoms[i][atomvalue];
              conjugate[i][positioninatoms] = i;

              cohabitant[i][leftatom]  = atoms[0][rightatom];
              cohabitant[i][rightatom] = atoms[i][rightatom];
            ...
            if ((atoms[0][rightatom] == atoms[i][leftatom])   &&
              ...
              cohabitant[i][leftatom]  = atoms[0][leftatom];
              cohabitant[i][rightatom] = atoms[i][rightatom];
            ...
            if ((atoms[0][leftatom] != atoms[i][leftatom])    &&
              ...
              cohabitant[i][leftatom]  = atoms[0][leftatom];
              cohabitant[i][rightatom] = atoms[i][leftatom];
...
#pragma omp barrier
          /* PARALLELISATION ADJUSTMENT */
          /* IF PLACED HERE, THE LIMIT CONTAINS ALSO TRIANGLE DOUBLETTS! */
          /* adjust triangulation to be only up to maximum triangles */
          j = 0;
          //#pragma omp parallel for
          for (i = 1; i < atomcount; i++) {
            if ((conjugate[i][atomnumber] != 0) && j <= trilimit) {
              maxtriangles = j;
              j++;
            } else
            if ((conjugate[i][atomnumber] != 0) && j > trilimit) {

              cohabitant[i][atomnumber]      = 0;
              cohabitant[i][leftatom]        = 0;
              cohabitant[i][rightatom]       = 0;
              cohabitant[i][atomvalue]       = 0;
              cohabitant[i][positioninatoms] = 0;

              conjugate[i][atomnumber]       = 0;
              conjugate[i][leftatom]         = 0;
              conjugate[i][rightatom]        = 0;
              conjugate[i][atomvalue]        = 0;
              conjugate[i][positioninatoms]  = 0;
...
#pragma omp barrier
          ...
          /* SEGFAULT #pragma omp parallel for */
          for (j = 0; j < maxtriangles; j++) {
            ...
            for (i = 1; i < atomcount; i++) {
              if ((((cohabitant[j][leftatom]  == atoms[i][leftatom]) &&
                ...
                /* #pragma omp parallel for -- optional instead of "upper" for */
                for (k = 0; k < j; k++) {
