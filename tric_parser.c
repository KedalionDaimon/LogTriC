...
#define lowestatom 2001
#define atomcount 1000
#define leninput 100
#define atomnumber 0
#define leftatom 1
#define rightatom 2
...
for (chargehistory = 0; chargehistory < MAXHISTORY; chargehistory++) {
  fscanf(historydata, "%d", &lasthistory[chargehistory]);
}
...
for (i = 0; i < atomcount; i++) {
  fscanf(atomdata, "%d", &atoms[i][atomnumber]);
  fscanf(atomdata, "%d", &atoms[i][leftatom]);
  fscanf(atomdata, "%d", &atoms[i][rightatom]);
  fscanf(atomdata, "%d", &atoms[i][atomvalue]);
}
fclose(atomdata);
...
for (imptrac = 0; imptrac < WORDSKNOWN; imptrac++) {
  fscanf(importancedata, "%d", &importantwords[imptrac]);
}
        ...
        */
        ...
        for (i = 0; i < maxtriangles; i++) {
          ...
          if (!((conjugate[i][atomnumber] == 0) && (cohabitant[i][atomnumber] == 0))) {
            ...
            if (((atoms[i][atomvalue] >= 0) && (conjugate[i][atomvalue] >= 0)) ||
                ((atoms[i][atomvalue] < 0) && (conjugate[i][atomvalue] < 0))) {
        
              cohabitanteffects[i] = sqrt(1.0 * atoms[i][atomvalue] * conjugate[i][atomvalue]);
              ...
              cohabitanteffects[i] = -1 * sqrt(-1.0 * atoms[i][atomvalue] * conjugate[i][atomvalue]);
            ...
            if (((atoms[i][atomvalue] >= 0) && (cohabitant[i][atomvalue] >= 0)) ||
                ((atoms[i][atomvalue] < 0) && (cohabitant[i][atomvalue] < 0))) {
        
              conjugateeffects[i] = sqrt(1.0 * atoms[i][atomvalue] * cohabitant[i][atomvalue]);
              ...
              conjugateeffects[i] = -1 * sqrt(-1.0 * atoms[i][atomvalue] * cohabitant[i][atomvalue]);
